export function SelectFilter({name, labelText, values, id, prefix ="", sufix="", onChangeSelect}) {
    return (
        <div className="flex items-center gap-2">
            <label className="sr-only text-slate-500 text-sm" htmlFor={id}>{labelText}</label>
            <div className="relative">
                <select defaultValue="" name={name} onChange={onChangeSelect}
                    ...
                    <option value="">{labelText}</option>
                    {
                        values.map((value, index) => (
                            <option key={index} value={value}>{prefix}{value}{sufix}</option> 
                        ))
                    }
                </select>
                <span
